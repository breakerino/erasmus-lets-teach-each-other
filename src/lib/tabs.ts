export const initializeTabs = (wrapperSelector) => {
	...
	for (const [index, tabButton] of tabButtons.entries()) {
		...
		const activeTabContent = tabWrapper.querySelector(`[data-tab-content][data-tab-id=${activeTabID}]`)
		...
		tabButton.addEventListener('click', (e) => {
			const activeTabButton = tabWrapper.querySelector(`[data-tab-button].active`)
			...
			tabWrapper.querySelectorAll(`[data-tab-content]`).forEach(tabContent => {
